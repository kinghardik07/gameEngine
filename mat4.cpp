...
/*matrix cofactorMatrix(matrix input_matrix,std::vector<int> index_position){
return scalarMultiplication(pow(-1,index_position[0]+index_position[1]),minorMatrix(input_matrix,index_position));
}*/
...
//make detOfPSC(post-square-check)
...
//if(isSquareMatrix(input_matrix)){
	...
	double det=0;
	for(int i=0;i<input_matrix[0].size();i++){
		det+=pow(-1,i)*input_matrix[0][i]*determinantOf(minorMatrix(input_matrix,{0,i}));
	}
	return det;
//}
}
...
//if(isSquareMatrix(input_matrix)){
	...
	for(int i=0;i<input_matrix.size();i++){
		for(int j=0;j<input_matrix[0].size();j++){
			...
			if(i==0) det+=input_matrix[0][j]*adjugate[0][j];
		}
		if(det==0) return NULL_MATRIX;	//singular matrix
	}
