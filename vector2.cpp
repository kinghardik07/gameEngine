...
double magnitudeOfVector(std::vector<double> input_vector,int d=VECTOR_DIMENSION){
    double magnitude = 0;
    for(int i=0;i<d;i++){
...
std::vector<double> addVector(std::vector<std::vector<double>> vector_vector){
    std::vector<double> resultant_vector (vector_vector[0].size());
    std::cout << "In addVector\nSize of vector_vector: " << vector_vector.size() <<std::endl;
    for(int i=0;i<vector_vector[0].size();i++){
